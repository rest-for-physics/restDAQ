...
TRestDAQGUI::TRestDAQGUI(const int &p,const int &q, std::string decodingFile){
  ...
  if(nStrips)hitmap = new TH2I("HitMap","HitMap",(nStrips/2)/+1,0,nStrips,(nStrips/2)+1,0,nStrips);
  else hitmap = new TH2I();
...
void TRestDAQGUI::cfgButtonPressed(){
  ...
  cfgMain->AddFrame(mb, new TGLayoutHints(kLHintsTop | kLHintsLeft | kLHintsExpandX, 0, 0, 1, 1));
   ...
   fContents->Connect("DoubleClicked(TGFrame*,Int_t)", "TRestDAQGUI", this,
                      "cfgButtonDoubleClick(TGLVEntry*,Int_t)");
...
void TRestDAQGUI::LoadLastSettings(){
  ...
  if(file.is_open() )file>>cfgFileName>>type>>nEvents;
  else std::cout<<"Cannot open "<<fileN<<std::endl;
}
...
void TRestDAQGUI::UpdateOutputs( ){
  ...
  if(tmp !=counterLabel->GetText()->Data())
    counterLabel->SetText(tmp.c_str());
  ...
  if(tmp !=rateLabel->GetText()->Data())
    rateLabel->SetText(tmp.c_str());

  tmp = "STATUS: ";
    if(status==0) tmp += "STOPPED";
    else if (status==1)tmp += "RUNNING";
    else tmp += "UNKNOWN";

  if(tmp !=statusLabel->GetText()->Data())
    statusLabel->SetText(tmp.c_str());
...
void TRestDAQGUI::READ(){
  ...
  while(!exitGUI){
    ...
    if (status == 1){
      ...
      hitmap->Clear();
      hitmap->Reset();
      instantRateGraph->Set(0);instantRateGraph->SetPoint(0,tNow,0);
      meanRateGraph->Set(0);meanRateGraph->SetPoint(0,tNow,0);
      rateGraphCounter=0;
