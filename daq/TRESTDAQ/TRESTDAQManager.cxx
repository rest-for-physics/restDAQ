...
void TRESTDAQManager::dataTaking( ){
  ...
  auto rT = daq_metadata_types::acqTypes_map.find(sM->runType);
    if(rT != daq_metadata_types::acqTypes_map.end()){
      ...
      std::cout<<"Warning: Acquisition type not found in shared memory, assuming the "<<daqMetadata.GetAcquisitionType().Data()<<" from config file"<<std::endl;
  ...
  auto eT = daq_metadata_types::electronicsTypes_map.find(elType);
    if(eT == daq_metadata_types::electronicsTypes_map.end()){
      ...
      std::cout<<"Valid electronics types:"<<std::endl;
        for(const auto & [name, t] : daq_metadata_types::electronicsTypes_map)
          std::cout<<(int)t<<" "<<name <<std::endl;
    } else {
...
void TRESTDAQManager::DetachSharedMemory(sharedMemoryStruct **sM){
  shmdt((sharedMemoryStruct*)*sM);
}
