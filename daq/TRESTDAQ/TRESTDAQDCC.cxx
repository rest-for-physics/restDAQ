...
TRESTDAQDCC::TRESTDAQDCC(TRestRun *rR, TRestRawDAQMetadata *dM) : TRESTDAQ(rR,dM){
  initialize();
}
...
void TRESTDAQDCC::configure() {
  ...
  if(GetDAQMetadata()->GetAcquisitionType() == "pedestal")SendCommand("pokes 0x14 0x400");//Set SCA readback offset
  else SendCommand("pokes 0x14 0xc00");//Set SCA readback offset
...
// If data supplied, copy to temporary buffer
  if (size <= 0) return;
	...
	if (GetDAQMetadata()->GetVerboseLevel() >= REST_Debug)std::cout<<"FEC "<<fec<<" asic "<<asic<<" channel "<<channel<<" physChann "<<physChannel<<"\n";
...
void DccSocket::Open(int *rem_ip_base, int rpt){
  ...
  timeout.tv_usec = 0;
    ...
    if (rcvsz_done < rcvsz_req){
      std::cout<<"Warning in socket: recv buffer size set to "<< rcvsz_done <<" bytes while "<< rcvsz_req <<" bytes were requested. Data losses may occur"<<std::endl;
    }
