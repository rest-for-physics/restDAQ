...
void TRESTDAQFEMINOS::pedestal() {
    ...
    // Event generator
    BroadcastCommand("event_limit 0x3",
                     FEMArray);  // # Event limit: 0x0:infinite; 0x1:1; 0x2:10; 0x3:100; 0x4: 1000; 0x5:10000; 0x6:100000; 0x7:1000000
...
void TRESTDAQFEMINOS::ReceiveThread(std::vector<FEMProxy>* FEMA) {
    ...
    for (auto& FEM : *FEMA) {
        ...
        if (FEM.client > smax) smax = FEM.client;
    }
    smax++;
    ...
    while (!stopReceiver) {
        ...
        for (auto& FEM : *FEMA) {
            if (FD_ISSET(FEM.client, &readfds_work)) {
                ...
                if (verboseLevel >= TRestStringOutput::REST_Verbose_Level::REST_Debug)
                    std::cout << "Packet received with length " << length << " bytes" << std::endl;
