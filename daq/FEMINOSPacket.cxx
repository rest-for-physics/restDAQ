    ...
    do {
        if ((*fr & PFX_14_BIT_CONTENT_MASK) == PFX_CARD_CHIP_CHAN_HIT_IX) {
            ...
            printf("Server RX stat: cmd_count=%d daq_req=%d daq_timeout=%d daq_delayed=%d daq_missing=%d cmd_errors=%d\n", tmp_i[0], tmp_i[1],
                   tmp_i[2], tmp_i[3], tmp_i[4], tmp_i[5]);
...
bool FEMINOSPacket::GetNextEvent(std::deque<uint16_t>& buffer, TRestRawSignalEvent* sEvent, uint64_t& tS, uint32_t& ev_count) {
    ...
    while (!endOfEvent && !buffer.empty()) {
        ...
        if ((buffer.front() & PFX_4_BIT_CONTENT_MASK) == PFX_START_OF_EVENT) {
            ...
            // printf( "Card %02d Chip %01d Channel_Hit_Count %02d\n", GET_CARD_IX(buffer.front()), GET_CHIP_IX(buffer.front()),
            // GET_CHAN_IX(buffer.front()) );
            buffer.pop_front();
        } else if ((buffer.front() & PFX_12_BIT_CONTENT_MASK) == PFX_CHIP_LAST_CELL_READ) {
            for (int i = 0; i < 4; i++) {
                // printf( "Chip %01d Last_Cell_Read %03d (0x%03x)\n",GET_LST_READ_CELL_CHIP_IX(buffer.front()), GET_LST_READ_CELL(buffer.front()),
                // GET_LST_READ_CELL(buffer.front()));
                buffer.pop_front();
            }
            ...
            while ((buffer.front() & PFX_12_BIT_CONTENT_MASK) == PFX_ADC_SAMPLE || (buffer.front() & PFX_9_BIT_CONTENT_MASK) == PFX_TIME_BIN_IX) {
            ...
            if (buffer.front() != 0) printf("WARNING: word : 0x%x (%d) unknown data\n", buffer.front(), buffer.front());
            buffer.pop_front();
        }
    }
...
bool FEMINOSPacket::isDataFrame(uint16_t* fr) { return ((*fr & PFX_9_BIT_CONTENT_MASK) == PFX_START_OF_DFRAME); }
